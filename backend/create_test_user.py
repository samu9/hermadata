    ...
    try:
        ...
        with sessionmaker() as session:
            ...
            service.user_repository.update(
                user.id,
                UpdateUserModel(is_superuser=True, is_active=True),
            )
            session.commit()

            print("✅ Created test super user:")
            print(f"   Email: {email}")
            print(f"   Password: {password}")
            print("   Super User: Yes")
            print(f"   User ID: {user.id}")

    except Exception as e:
        print(f"❌ Error creating test user: {e}")
        print(
            "This might be because the user already exists or the database is "
            "not set up."
        )
