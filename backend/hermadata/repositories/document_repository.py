...
class SQLDocumentRepository(BaseRepository):
    ...
    def new_document_kind(self, name: str):
        result = self.session.execute(insert(DocumentKind).values(name=name))
        self.session.commit()
        new_kind = DocKindModel(id=result.lastrowid, name=name)
        return new_kind
