...
def new_document(doc: UploadFile, doc_repo: Annotated[SQLDocumentRepository, Depends(document_repository)]):
...
def get_document_kinds(doc_repo: Annotated[SQLDocumentRepository, Depends(document_repository)]):
    kinds = doc_repo.get_document_kinds()
    return kinds


@router.post("/kind", response_model=DocKindModel)
def create_new_kind(data: NewDocKindModel, doc_repo: Annotated[SQLDocumentRepository, Depends(document_repository)]):
    try:
...
def serve_document(document_id: int, doc_repo: Annotated[SQLDocumentRepository, Depends(document_repository)]):
