...
def test_vet_repository_create(db_session: Session, vet_repository: SQLVetRepository):
    data = VetModel(business_name="Veterinario 1", fiscal_code="1234567890a")

    result = vet_repository.create(data)

    check = db_session.execute(select(Vet).where(Vet.id == result.id)).scalar_one()
